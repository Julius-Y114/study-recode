import torch
from networkx.algorithms.isomorphism.isomorph import faster_could_be_isomorphic
...
class NameDataset(Dataset):
    def __init__(self,is_train_set=True):
        filename = './Samples/names_train.csv.gz' if is_train_set else './Samples/names_test.csv.gz'
...
class RNNClassifier(torch.nn.Module):
    ...
    def forward(self,input,seq_lengths):
        ...
        if self.n_directions == 2:
            hidden_cat = torch.cat([hidden[-1],hidden[-2]],1)
        else:
            hidden_cat = hidden[-1]
...
def trainModel():
    ...
    for i,(name,countries) in enumerate(trainloader,1):
        ...
        if i % 10 == 0:
            print(f'[{time_since(time.time())}] Epoch {epoch}',end='')
...
def testModel():
    ...
    with torch.no_grad():
        ...
        percentage = '%.2f' % (100 * correct / total)
        print(f'Accuracy on test set: {correct}/{total}  {percentage}%')
